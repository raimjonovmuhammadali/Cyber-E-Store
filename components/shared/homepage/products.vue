            ...
            <div class="w-full p-4">
                <div v-for="(tab, index) in tabs" :key="index" v-show="activeTab === index" class="w-full h-auto">
                    <div class="w-full flex items-center justify-between gap-3 text-center flex-wrap">
                        <div v-for="product in products[tab.value]" :key="product.id" class="w-[45%] md:w-[23%] h-[352px] p-4 border rounded-[9px] shadow flex flex-col items-center justify-center gap-2 bg-[#F6F6F6]">
                            <img src="~/assets/img/Iphone 14 pro 1.png" alt="Product Image" class="w-[104px] h-[104px] md:w-[160px] md:h-[160px] object-cover mb-4" />
                            <h3 class="text-[16px] font-[500]">{{ product.name }}</h3>
                            <p class="mt-2 font-bold">${{ product.price }}</p>
                            <span class="w-[139px] md:w-[188px] rounded-[8px] py-2 px-2 flex items-center justify-center cursor-pointer bg-black text-white">Buy Now</span>
                        </div>
                    </div>
                </div>
            </div>
